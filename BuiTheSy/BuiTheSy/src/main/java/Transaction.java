...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput i : inputs) {
            i.UTXO = BTS_Blockchain.UTXOs.get(i.transactionOutputId);
        }

        //Kiểm tra giao dịch hợp lệ
        if(getInputsValue() < BTS_Blockchain.minimumTransaction) {
            System.out.println("Đầu vào của giao dịch quá bé: " + getInputsValue());
            System.out.println("Vui lòng nhập số lượng giao dịch lớn hơn " + BTS_Blockchain.minimumTransaction);
            return false;
        }
        ...
        outputs.add(new TransactionOutput( this.reciepient, value,transactionId)); //Gửi số lượng đến kho nhận
        outputs.add(new TransactionOutput( this.sender, leftOver,transactionId)); //Gửi số lượng còn lại về kho gửi

        //Đưa giao dịch vào UTXO
        for(TransactionOutput o : outputs) {
            BTS_Blockchain.UTXOs.put(o.id , o);
        }
        ...
        for(TransactionInput i : inputs) {
            ...
            BTS_Blockchain.UTXOs.remove(i.UTXO.id);
        }
